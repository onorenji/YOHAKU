...
while diff != 0 and i < num_people:
    ...
    if (step > 0) or (rounded_amounts[idx] >= 1000):  # マイナス調整時に0円以下にならないように
        rounded_amounts[idx] += step
        diff -= step
    i = (i + 1) % num_people  # ループで順番に調整
...
for i in range(num_people):
    st.write(f"{names[i]}（階級 {ranks[i]}）: {final_amounts[i]} 円")